...
class GBNHost():
    ...
    def create_ack_pkt(self, seq_num):
        ...
        prechksum_ack = pack('!HIH', pkt_type, seq_num, placehold_chksum)

        checksum = self.create_checksum(prechksum_ack)

        ack_packet = pack('!HIH', pkt_type, seq_num, checksum)
    ...
    def unpack_pkt(self, packet):
        ...
        """
        p_t, s_n, chk = unpack('!HIH', packet[:8])
        packet = packet[8:]
        ...
        p_l = unpack('!I', packet[:4])[0]
        packet = packet[4:]
        payload = packet.decode()

        packet_data.update({'payload_length': p_l})
